You are reviewing a pull request for beamtalk_watcher: a monitoring tool written in BeamTalk. It polls HTTP, TCP and
BEAM-process health checks, routes outbound state-change notifications through a deduping
fan-out router, accepts inbound webhook alerts, and serves a live dashboard with a JSON API.

BeamTalk is a Smalltalk-inspired language that compiles to the BEAM (Erlang VM). This is a
standalone package repo: sources live in `src/*.bt` (with `checks/`, `dashboard/`, `notifications/` and `receiver/`
subdirectories), BUnit tests in `test/*_test.bt`, the design is written up in `SPEC.md`, and
the package manifest is `beamtalk.toml`. The BeamTalk compiler, runtime and stdlib live in the
...
DOMAIN LENSES:
- Check-and-notify lifecycle: a flapping check must not fan out duplicate notifications, and
  the deduping router must not swallow a genuine state change. Check timer and poll cleanup
  on stop, the inbound webhook receiver against malformed and oversized payloads, and that
  one failing check cannot take down the dashboard or the other checks.
